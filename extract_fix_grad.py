...
def grad(job,newdir):
    ...
    # CFOUR Gradient
        ...
        # Get it from real output
        output=output.split('.')[0]+'.out'
        f=open(output,'r')
        while True:
            line=f.readline()
        ...
        for i in range(job.natom):
            ...
            print arry
            if len(arry)==5: offset=2
            if len(arry)==4: offset=1
            for j in range(3):
                grad[atmap[i]][j]=float(arry[j+offset])
        grad.shape=(natom,3)
        job.grad=grad
        exlogger.info('\nGradient from %s\n%s' %(job.exprog,job.grad))
        f.close()
