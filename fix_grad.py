...
import logging,assemble,subprocess
import extract_fix_grad,extract
import newass
...
# Create trimer input
if level>2:
    trimers=[ i + j + k for i in mons for j in mons for k in \
        mons if i<j<k ]
...
# Create tetrad input
if level>3:
    tetrads=[ i + j + k + l for i in mons for j in mons for k in mons \
        for l in mons if i<j<k<l ]
...
#        i.runInput(newdir,scrdir)
        extract_fix_grad.energy(i,newdir)
        if deriv>0:
            extract_fix_grad.grad(i,newdir)
            if deriv>1:
                extract_fix_grad.hessian(i,newdir)
